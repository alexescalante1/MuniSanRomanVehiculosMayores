...
export class FragmentData {
  ...
  constructor(dtK: string) {
    ...
    // this.CfiBs.push("Hola");
    // this.CfiBs.push("Como estas");

    // new LocalStorageAdapter().set("VarTemp", { Value: JSON.stringify(this.CfiBs) });
    // console.log((new LocalStorageAdapter()
    // .get("VarTemp")
    // ?.Value)?.toString());
    ...
    try {
      if (this.C4ss1628989 === "MyKingAlex") {
        ...
        for (let i = 0; i < this.Vrbls.length; i++) {
          new LocalStorageAdapter().set(this.Vrbls[i], {
            Value: this.Dtbls[i],
          });
        }
        return true;
  ...
  GtParttnDtLS = (mky: string, idPtc: string): string => {
    try {
      if (this.C4ss1628989 === "MyKingAlex") {
        ...
        this.DtVarFragment(mky, idPtc);
        for (let i = 0; i < this.Vrbls.length; i++) {
          DtEncrypt += new LocalStorageAdapter()
            .get(this.Vrbls[i])
            ?.Value?.toString();
        }
