...
export class GetInitConfigCls {
  ...
  async FormatOuto(headers: any): Promise<any> {
    ...
    const httpResponseMenu = await new AxiosHttpClient().Request({
      url: this.url,
      method: this.method,
      headers: headers,
    });
    ...
    new AlertServerError().AlertStatusCode(httpResponseMenu.statusCode);
  ...
  async FormatListMenuNavbar(headers: any, ObjData: any): Promise<any> {
    ...
    if (ObjData) {
      for (let i = 0; i < ObjData.length; i++) {
        const httpResponseMenuAside = await new AxiosHttpClient().Request({
        ...
        new AlertServerError().AlertStatusCode(
